...
def resnet50():
    ...
    a = resnet_block(x, 64, [1,1], 1)
    b = resnet_block(a, 64, [3,3], 1)
    c = resnet_block(b, 256, [1,1], 1, activation = None)
    x=Conv2D(256,kernel_size=[1,1],strides=1,padding='same',kernel_initializer='he_normal',kernel_regularizer=l2(1e-3))(x)
    ...
    c = resnet_block(b,1024,[1,1],1, activation = None)
    x=Conv2D(1024,kernel_size=[1,1],strides=2,padding='same',kernel_initializer='he_normal',kernel_regularizer=l2(1e-3))(x)
    x=keras.layers.add([x,c])
    x=Activation('relu')(x)
    
    for i in range(5):
        a = resnet_block(x, 256, [1,1], 1)
        b = resnet_block(a, 256, [3,3], 1)
        c = resnet_block(b, 1024, [1,1], 1, activation = None)
        x=keras.layers.add([x,c])
        x=Activation('relu')(x)
    ...
    c = resnet_block(b,2048,[1,1],1)
    x=Conv2D(2048,kernel_size=[1,1],strides=2,padding='same',kernel_initializer='he_normal',kernel_regularizer=l2(1e-3))(x)
    x=keras.layers.add([x,c])
    x=Activation('relu')(x)

    for i in range(5):
        a = resnet_block(x, 512, [1,1], 1)
        b = resnet_block(a, 512, [3,3], 1)
        c = resnet_block(b, 2048, [1,1], 1, activation = None)
        x=keras.layers.add([x,c])
        x=Activation('relu')(x)
    #print(x.get_shape())
...
def getDataAndSize():
    ...
    for dat in data:
      indexId+=1
      path = dat[0]
      image=Image.open("resizedImages/"+path).convert('RGB')
...
def getBoxes():
  ...
  for dat in data:
      ...
      box[0]=(dat[1]/640)*224
      box[1]=(dat[3]/480)*224
      box[2]=((dat[2]-dat[1])/640)*224
      box[3]=((dat[4]-dat[3])/480)*224
...
def getdata():
    ...
    # print(data)
    data=data[index]
    data_train=data[0:23950]
    data_test=data[23950:]
    box = getBoxes()
    box=box[index]
    box_train=box[0:23950]
    box_test=box[23950:]
...
checkpoint=ModelCheckpoint('finalModel.h5',monitor='val_loss',verbose=0,save_best_only=True,mode='auto')


# In[ ]:


model_details=model.fit(data_train,box_train,batch_size=64,epochs=140,shuffle=True,validation_split=0.1,callbacks=[lr_scheduler,lr_reducer,checkpoint],verbose=1)
